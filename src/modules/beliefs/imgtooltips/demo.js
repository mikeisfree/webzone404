function verifyImagePaths() {
  ...
  links.forEach((link) => {
    const img = new Image();
    img.onload = () => console.log("✅ Image loaded:", link.dataset.img);
    img.onerror = () =>
      console.error("❌ Image failed to load:", link.dataset.img);
    img.src = link.dataset.img;
  });
}

function debugImageLoading(img) {
...
document.addEventListener("DOMContentLoaded", () => {
  console.log("DOM loaded, setting up image tooltips...");

  // Check if elements exist
  const blockTitles = document.querySelectorAll(".block__title");
  console.log("Found block titles:", blockTitles.length);

  // Check data-img attributes
  blockTitles.forEach((title) => {
    console.log("Title element:", title);
    console.log("Image path:", title.getAttribute("data-img"));

    // Add explicit hover listener
    title.addEventListener("mouseenter", (e) => {
      console.log("Hover started on:", e.target);
      console.log("Should show image:", e.target.getAttribute("data-img"));
    });
  });

  // Preload images
  const contentel =
    document.querySelector(".beliefs") || document.createElement("div");
  const preloadImages = () => {
    [...document.querySelectorAll(".block__title, .block__link")].forEach(
      (el) => {
        if (!el.dataset.img) {
          console.warn("Missing data-img attribute for element:", el);
          return;
        }
  ...
  if (typeof imagesLoaded !== "undefined") {
    console.log("imagesLoaded available, initializing...");
...
// Add this to the beginning of your demo.js file or in a script tag
console.log("DOM loaded, setting up image tooltips...");
...

{
  ...
  // Replace the last part of the code with this:
  document.addEventListener("DOMContentLoaded", () => {
    // Demo purpose only: Preload all the images in the page..
    const contentel =
      document.querySelector(".beliefs") || document.createElement("div");

    // Add contentel to document body if it's not already there
    if (!contentel.parentNode) {
      document.body.appendChild(contentel);
    }

    // Create a preloader container
    ...
    const preloadPromises = [
      ...document.querySelectorAll(".block__title, .block__link"),
    ].map((el) => {
      if (!el.dataset.img) {
        console.warn("Missing data-img attribute for element:", el);
      ...
      return Promise.all(
        imgsArr.map((imgSrc) => {
          return new Promise((resolve, reject) => {
            ...
            imgel.src = imgSrc;
            imgel.className = "preload";
            preloaderContainer.appendChild(imgel);
          });
        })
      );
    });
    ...
    Promise.all(preloadPromises.flat())
      .then(() => {
        console.log("All images loaded successfully");
        if (document.body) {
          document.body.classList.remove("loading");
      ...
      .finally(() => {
        ...
        [
          ...
          // ...existing effects...
          ["23", HoverImgFx23],
        ].forEach(([fx, EffectClass]) => {
          [
            ...document.querySelectorAll(
              `[data-fx="${fx}"] > a, a[data-fx="${fx}"]`
            ),
          ].forEach((link) => new EffectClass(link));
        });
      });
  });
}
